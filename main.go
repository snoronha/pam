...
func main() {
    ...
    anomalyCount := map[string]int{
        ...
        "ZERO_CURRENT_V3": 0, "ZERO_CURRENT_V4": 0,  "PF_SPIKES_V3": 0,
        "ZERO_POWER_V3": 0,   "ZERO_POWER_V4": 0,
        "ZERO_VOLTAGE_V3": 0, "ZERO_VOLTAGE_V4": 0,  "THD_SPIKES_V3": 0,
    }
    processAnomaly := map[string]bool{
        ...
        "ZERO_CURRENT_V3": true,  "ZERO_CURRENT_V4": true,  "PF_SPIKES_V3": true,
        "ZERO_POWER_V3": true,    "ZERO_POWER_V4": true,
        "ZERO_VOLTAGE_V3": true,  "ZERO_VOLTAGE_V4": true,  "THD_SPIKES_V3": true,
    }
    for _, f  := range files {
        filePath := dir + "/" + f.Name()
        if strings.Contains(f.Name(), ".csv") {
            if fileNum > 222 {
            // if fileNum >= 0 {
                processEDNAFile(filePath, fileNum, startTime, anomalyCount, processAnomaly)
            }
            fileNum++
        }
    }
...
func processEDNAFile(fileName string, fileNum int, startTime time.Time, anomalyCount map[string]int, processAnomaly map[string]bool) {
    ...
    // headers  :=  map[string]int{ "ExtendedId": 0, "Time": 1, "Value": 2, "ValueString": 3, "Status": 4 }
                ...
                if (processAnomaly["ZERO_CURRENT_V3"] || processAnomaly["ZERO_CURRENT_V4"]) &&
                    ...
                    if value > -0.5 && value < 1 {
                        if zeroCurrentWindow.quantileGreaterThanThreshold(0.01, 10.0) {
                            // fmt.Printf("%d: [%d, %d] value: %.2f\n", numLines, zeroCurrentWindow.startPointer, zeroCurrentWindow.endPointer, value)
